...
class FCS(BaseLearner):
    ...
    def _class_aug(self, inputs, targets, alpha=20., mix_time=4, inputs_aug=None):
        """
        HCVR 条件消融逻辑：
        - Semantic (语义): 基础输入 (始终存在)
        - Geometric (几何): Rotation 变换 (由 enable_rot 控制)
        - Synthetic (合成): Mixup 变换 (由 enable_mix 控制)
        """
        enable_rot = self.args.get("enable_rot", True)
        enable_mix = self.args.get("enable_mix", True)

        # --- 1. 处理几何条件 (Geometric / Rotation) ---
        if enable_rot:
            # 执行 4 个角度的旋转映射
            inputs_rot = torch.stack([torch.rot90(inputs, k, (2, 3)) for k in range(4)], dim=1)
            inputs_rot = inputs_rot.view(-1, 3, inputs_rot.shape[-2], inputs_rot.shape[-1])
            targets_rot = torch.stack([targets * 4 + k for k in range(4)], dim=1).view(-1)

            # 对对比学习用的增强图像同步旋转
            inputs_aug_rot = torch.stack([torch.rot90(inputs_aug, k, (2, 3)) for k in range(4)], dim=1)
            inputs_aug_rot = inputs_aug_rot.view(-1, 3, inputs_aug_rot.shape[-2], inputs_aug_rot.shape[-1])
        else:
            # 不开启几何条件，仅保留原始 0 度图像，但需适配 4k 标签空间 (k=0)
            inputs_rot = inputs
            targets_rot = targets * 4 
            inputs_aug_rot = inputs_aug

        # --- 2. 处理合成条件 (Synthetic / Mixup) ---
        mixup_inputs = []
        mixup_targets = []
        ...
        if enable_mix:
            for _ in range(mix_time):
                ...
                # 使用 Beta 分布生成插值系数 (alpha_mix 即此处 alpha)
            ...
            # 最终合并：几何分量 + 合成分量
            final_inputs = torch.cat([inputs_rot, mixup_inputs], dim=0)
            final_targets = torch.cat([targets_rot, mixup_targets], dim=0)
        else:
            # 不开启合成条件，仅返回几何/语义部分
            final_inputs = inputs_rot
            final_targets = targets_rot

        return final_inputs, final_targets, inputs_aug_rot
